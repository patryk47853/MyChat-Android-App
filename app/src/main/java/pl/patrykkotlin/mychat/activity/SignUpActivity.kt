            ...
            if (task.isSuccessful) {
                ...
                firestore.collection("Users").document(user?.uid ?: "").set(dataHashMap)
                    ...
                    .addOnFailureListener { e ->
                        progressDialog.dismiss()
                        Toast.makeText(this, "Registration process failed: ${e.message}", Toast.LENGTH_SHORT).show()
                    }
            } else {
                progressDialog.dismiss()
                Toast.makeText(this, "Registration process failed: ${task.exception?.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
}